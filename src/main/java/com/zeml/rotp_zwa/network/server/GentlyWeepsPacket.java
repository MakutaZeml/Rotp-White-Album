...
public class GentlyWeepsPacket {
    private final int entityID;
    private final boolean gentlyWeeps;

    public GentlyWeepsPacket(int entityID, boolean stay){
        this.entityID = entityID;
        this.gentlyWeeps = stay;
    }


    public static void encode(GentlyWeepsPacket msg, PacketBuffer buf) {
        buf.writeInt(msg.entityID);
        buf.writeBoolean(msg.gentlyWeeps);
    }

    public static GentlyWeepsPacket decode(PacketBuffer buf) {
        return new GentlyWeepsPacket(buf.readInt(), buf.readBoolean());
    }


    public static void handle(GentlyWeepsPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
